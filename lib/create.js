...
var createScript = function (ruleset) {
  ...
  let string_evaluator = rules.reduce((script, test) => {
    ...
    let and_test = /and||simple/i.test(rule_type);
    let eval_group;
    ...
    if (or_test) {
      script += `_global.output_types['${rule_name}'] = {'rule_type': '${rule_type}', 'decline_reason': ${handleValueAssignment(decline_reason)}};\r\n`;
    }
    if (and_test) {
      script += `_global.output_types['${rule_name}'] = {'rule_type': '${rule_type}', 'decline_reason': ${handleValueAssignment(decline_reason)}};\r\n`;
    }
  ...
  return function evaluator(state) {
    ...
    try {
      ...
      context._global.rule_results.forEach(rule_result => {
        ...
        if (output_types[ rule_result.name ] && (output_types[ rule_result.name ].rule_type === 'AND' || output_types[ rule_result.name ].rule_type === 'simple')) {
